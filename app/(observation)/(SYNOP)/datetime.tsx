...
import { ChevronDownIcon, CloudSunRain } from "lucide-react-native";
...
export default function DateTime() {
    ...
    const getCurrentUTCXX00 = async (stationId?: number, date?: string) => {
        ...
        try {
            ...
            const synopData = await getLSynopData(db, stationId, undefined, date);

            const takenHours = synopData.map((d: any) => d.sHour);
    ...
    const navigateToDataCollectionScreen = () => {
        ...
        router.push({
            pathname: "/(observation)/(SYNOP)/data-collection",
            params: {
        ...
        <Box className="flex-1 p-8 bg-white">
            <Box className="gap-4 pt-4 rounded">
                <Center className="pt-4">
                    ...
                    <Box className="bg-gray-100 p-5 rounded-full mb-4">
                        <Icon as={CloudSunRain} size="xl" className="text-blue-400" />
                    </Box>

                    {/* Main Heading */}
                    <Heading size="2xl" className="font-extrabold text-blue-400 mb-2 text-center">
                        SYNOP
                    </Heading>

                    {/* Subtext / Description */}
                    <Text size="lg" className="text-gray-500 text-center">
                        Select the station and time for this report.
                    </Text>
                </Center>
                <Box className="border border-gray-200 px-6 py-8 rounded-xl shadow-lg bg-white gap-2">
                                ...
                                <SelectContent>
                                    ...
                                    <SelectVirtualizedList
                                        ...
                                        renderItem={({ item }: { item: Station }) => (
                                            <SelectItem
                                                label={`${item.wmoID}${item.stationID} - ${item.stnName}`}
                    ...
                    <Box className="flex flex-row gap-2 mb-6">
                        ...
                        <TimePicker time={String(time ?? "")} setTime={setTime} date={date} stationID={station?.Id} />
                    </Box>
